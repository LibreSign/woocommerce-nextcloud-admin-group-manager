...
import { buyTheNextcloudPlan, logInAsAdmin, openOrder } from '../support/store';

test.describe( 'Provisioning the admin group of a paid order', () => {
	test.beforeEach( async () => {
		await nextcloud.answerWith( 200 );
		await nextcloud.forget();
	} );

	test( 'creates the admin group on Nextcloud and completes the order', async ( { page } ) => {
		...
		expect( await nextcloud.requests() ).toEqual( [
			{
				...
				authorization: nextcloud.authorization(),
				body: {
	...
	test( 'keeps the order processing when Nextcloud refuses it until the admin retries the sync', async ( { page } ) => {
