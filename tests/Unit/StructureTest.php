...
use LibreSign\WooNextcloud\Tests\Support\PluginFiles;
use PHPUnit\Framework\TestCase;

final class StructureTest extends TestCase {

	private const PLUGIN_FILE = 'woocommerce-nextcloud-admin-group-manager.php';
	...
	private static function tests_covering( $file ) {
		...
		if ( str_starts_with( $file, 'includes/agm-' ) ) {
			return array( 'tests/Integration/Includes/' . self::studly( substr( $file, strlen( 'includes/agm-' ), -strlen( '.php' ) ) ) . 'Test.php' );
		}
	...
	private static function files_covered_by( $file ) {
		if ( str_starts_with( $file, 'tests/E2E/Includes/' ) ) {
			return array( 'includes/agm-' . self::kebab( substr( $file, strlen( 'tests/E2E/Includes/' ), -strlen( '.spec.ts' ) ) ) . '.php' );
		}
		...
		if ( str_starts_with( $name, 'Includes/' ) ) {
			return array( 'includes/agm-' . self::kebab( substr( $name, strlen( 'Includes/' ) ) ) . '.php' );
		}
